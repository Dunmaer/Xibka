...
void main(){
  ...
  // (alpha, not colour: the canvas is uploaded unpremultiplied, so its soft glow has full-white
  // colour and would swell every letter into a blob)
  float rune = texture2D(uRuneMap, vUv).a * uRunes;
  col = mix(col, col*0.55, rune*0.4);
  vec3 emit = uRuneCol * rune * (0.5 + 0.25*vnoise(vec2(uTime*4.0, vUv.x*30.0)));
