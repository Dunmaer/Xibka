...
import { buildStructure, pickStructure } from './structures';
import {
...
export function generateCircle(g: GeneratorInput): CircleDesign {
  ...
  // ---------- Large structure: which kind of figure this curse grows into
  const structure = pickStructure(rng.fork(7));
  let figure = '';
  ...
  // ---------- 9. Hanging inscriptions: rings framing the certificate at the end
  {
    ...
    // (around a structure the rings would bring the big circle back, so they are rare there)
    const count = structure === 'classic' ? hr.int(1, 2) : hr.chance(0.25) ? 1 : 0;
  ...
  return { palette, layers, passers, sigils, architecture: `${structure === 'classic' ? `classic/${figure}` : structure}/${arch}/${palette.name}` };
}
